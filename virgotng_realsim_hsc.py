...
def main():
    ...
    sim_tag = os.getenv('SIMULATION')
    njobs = int(os.getenv('JOB_ARRAY_NJOBS'))
    job_idx = int(os.getenv('JOB_ARRAY_INDEX'))
    

    snap_min = int(os.getenv('SNAPMIN'))
    snap_max = int(os.getenv('SNAPMAX'))
    ...
    project_path = f'/vera/ptmp/gc/bconn/SKIRT/{universe}/{sim_tag}/HSCSSP'
    # base path where TNG data is stored
    il_path = f'/virgotng/universe/{universe}/{sim_tag}/output'
    # photometry path
    ...
    for snap in snaps:
        subs,mstar = get_subhalos(il_path,snap,mstar_lower=10)
