...
def cutoutImage(butler, skymap, specObjID, ra, dec, bands, width=60., imgdir='.'):
    # width is in arcsec
    ...
    for band in bands:
        ...
        try:
			...
			for _patch, _bbox in zip(patches, bboxes):
				fname = butler.get('deepCoadd_calexp_filename',{'tract': tract, 'patch': '%d,%d' % (_patch), 'filter': band})[0]
...
#sub <<= exposure.getMaskedImage()
				...
				image_fname = imgdir+'/%s_%s.fits' % (specObjID, band)
...
if __name__ == '__main__':
	...
	specObjID='hello_world'
	ra = 16.6027
	dec = 0.549747
    cutoutImage(butler, skymap, specObjID, ra, dec, bands, 60,outdir='~/Projects/HSC/Cutouts')
